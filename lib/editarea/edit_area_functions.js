	...
	EditArea.prototype.check_undo= function(only_once){
		...
		if(this.textareaFocused && editAreas[this.id]["displayed"]==true){
			...
			if(!this.previous[this.previous.length-1] || this.previous[this.previous.length-1]["text"] != text){
				this.previous.push({"text": text, "selStart": this.textarea.offsetWidth > 0 ? this.textarea.selectionStart : 0, "selEnd": this.textarea.offsetWidth > 0 ? this.textarea.selectionEnd : 0});
	...
	EditArea.prototype.toggle_full_screen= function(to){
		...
		{	// toogle on fullscreen		
			...
			var frame= parent.document.getElementById("template_editor_content");
			...
			this.fullscreen['old_editor_height']= frame.style.height;
			if(this.nav['isOpera']){
...
//	alert(screen.height+"\n"+window.innerHeight+"\n"+html.clientHeight+"\n"+window.offsetHeight+"\n"+document.body.offsetHeight);
			...
			parent.window['fn_adjust_template_list'](html.clientHeight);
			...
			frame.style.top= "-"+(parent.calculeOffsetTop(frame) + 3)+"px";
			frame.style.left= "-"+(parent.calculeOffsetLeft(frame) + 3)+"px";
		...
		{	// toogle off fullscreen
			...
			frame.style.height= this.fullscreen['old_editor_height'];
			
			parent.window['fn_adjust_template_list'](parseInt(this.fullscreen['old_editor_height']));
