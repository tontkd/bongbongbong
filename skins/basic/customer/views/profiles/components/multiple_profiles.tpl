{* $Id: multiple_profiles.tpl 7388 2009-04-29 10:26:49Z zeke $ *}
...
<div class="form-field select-profile">
	...
	{foreach from=$user_profiles item="up" name="pfe"}
		...
		{else}
		<a {if $use_ajax}class="cm-ajax"{/if} href="{$config.current_url|fn_query_remove:"profile"|fn_link_attach:"profile_id=`$up.profile_id`"}" rev="checkout_steps,cart_items,checkout_totals">{$up.profile_name}</a>
		{/if}
		{if !$smarty.foreach.pfe.last}&nbsp;|&nbsp;{/if}

		{if $up.profile_type != "P" && !$hide_profile_delete}
			{include file="buttons/button.tpl" but_meta="cm-confirm" but_rev="checkout_steps,cart_items,checkout_totals" but_role="delete" but_text="&nbsp;" but_href="`$index_script`?dispatch=profiles.delete_profile&amp;profile_id=`$up.profile_id`"}
		{/if}
	{/foreach}
	{if !$skip_create}
		&nbsp;&nbsp;{$lang.or}&nbsp;&nbsp;&nbsp;{if $smarty.request.profile == "new"}<strong>{$lang.create_profile}</strong>{else}<a class="lowercase{if $use_ajax} cm-ajax{/if}" href="{if $create_href}{$create_href}{else}{$config.current_url|fn_query_remove:"profile_id"|fn_link_attach:"profile=new"}{/if}" rev="checkout_steps,cart_items,checkout_totals">{$lang.create_profile}</a>{/if}
	{/if}
</div>
{/if}
