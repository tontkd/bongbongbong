...
<tr>
	<th width="10%">
		{if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=date&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "date"}sort-link-{$search.sort_order}{/if}">{/if}{$lang.date}{if $no_sort != "Y"}</a>{/if}</th>
	<th class="center">
		{$lang.pages}</th>
	<th>
		{if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=ip&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "ip"}sort-link-{$search.sort_order}{/if} inline-block">{/if}{$lang.ip}{if $no_sort != "Y"}</a>{/if}/ {if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=proxy&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "proxy"}sort-link-{$search.sort_order}{/if} inline-block">{/if}{$lang.proxy}{if $no_sort != "Y"}</a>{/if}</th>
	{if $smarty.request.client_type == "B"}
	<th>
		{if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=robot&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "robot"}sort-link-{$search.sort_order}{/if}">{/if}{$lang.robot}{if $no_sort != "Y"}</a>{/if}</th>
	{else}
	<th>
		{if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=os&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "os"}sort-link-{$search.sort_order}{/if}">{/if}{$lang.operating_system}{if $no_sort != "Y"}</a>{/if}</th>
	<th>
		{if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=browser&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "browser"}sort-link-{$search.sort_order}{/if}">{/if}{$lang.browser}{if $no_sort != "Y"}</a>{/if}</th>
	{/if}
	<th>
		{if $no_sort != "Y"}<a rev="visitors_list" href="{$c_url}&amp;sort_by=country&amp;sort_order={$search.sort_order}" class="{$ajax_class} {if $search.sort_by == "country"}sort-link-{$search.sort_order}{/if}">{/if}{$lang.country}{if $no_sort != "Y"}</a>{/if}</th>
...
<tr>
	...
	<td class="center nowrap">
		{if $action != "pages"}<a href="{$index_script}?dispatch=statistics.visitor_pages&amp;client_type={$search.client_type}&amp;stat_sess_id={$visitor.sess_id}&amp;return_url={$config.current_url|escape:url}">{/if}{$visitor.requests_count|default:1}{if $action != "pages"}</a>{/if}</td>
	<td class="nowrap"><strong>{$visitor.host_ip}</strong> / {if $visitor.proxy_ip}{$visitor.proxy_ip}{else}-{/if}</td>
	{if $smarty.request.client_type == "B"}
	...
	<td class="nowrap">
		{if $visitor.browser}
			...
			{if $browser == "explorer" || $browser == "firefox" || $browser == "mozilla" || $browser == "netscape" || $browser == "safari" || $browser == "opera"}
		...
		{/if}</td>
	{/if}
	<td><img src="{$images_dir}/flags/{$visitor.country_code|default:"01"|lower}.png" width="16" height="16" border="0" alt="{$visitor.country}" title="{$visitor.country}" align="top" />&nbsp;{if $visitor.country}{$visitor.country}{elseif $visitor.country_code}{$visitor.country_code}{else}{$lang.undefined}{/if}</td>
	{if $hide_extra_button != "Y"}
	<td class="center nowrap">
...
<tr id="visitors_log_{$smarty.foreach.visitors.iteration}" {if $hide_extra_button != "Y"}class="hidden"{/if}>
	<td colspan="{if $smarty.request.client_type == "B"}6{else}7{/if}">
		...
		<div class="form-field">
			...
			{if $visitor.referrer}<a href="{$visitor.referrer}">{$visitor.referrer}</a>{else}-{/if}
