...
//
// $Id: statistics.php 7594 2009-06-18 13:06:51Z lexa $
//
...
require_once(DIR_CORE . 'fn.sales_reports.php'); // FIXME: move amchars to separate file
...
// Synchronize server and local time;
$sql_variables = db_get_array("SHOW variables LIKE '%time%zone'");
foreach ($sql_variables as $v) {
	if ($v['Variable_name'] == 'timezone' || $v['Variable_name'] == 'system_time_zone' ) {
		$server_timezone = $v['Value'];
		break;
	}
}

fn_define('TIME_DIFFERENCE', strtotime("January 2 1970 00:00:00") - strtotime("January 2 1970 00:00:00 $server_timezone"));
...
if ($mode == 'delete') {
	...
	return array(CONTROLLER_STATUS_OK, "$index_script?dispatch=statistics.reports");

//
// Get amChart's settings
//
} elseif ($mode == 'get_settings') {
	echo fn_get_amchart_settings($_REQUEST['type'], $_REQUEST['title'], $_REQUEST['setting_type']);
	exit;
...
//
	...
	$total = db_get_field("SELECT COUNT(*) FROM ?:stat_requests WHERE sess_id = ?i ORDER BY req_id", $_REQUEST['stat_sess_id']);
	$limit = fn_paginate($params['page'], $total, Registry::get('settings.Appearance.admin_elements_per_page'));
	$requests = db_get_array("SELECT req_id, url, title, timestamp, https FROM ?:stat_requests WHERE sess_id = ?i ORDER BY req_id DESC $limit", $_REQUEST['stat_sess_id']);

	fn_add_breadcrumb(fn_get_lang_var(($params['client_type'] == 'B' ? 'robots' : 'visitors') . '_log'), $_REQUEST['return_url']);
...
//
	...
	list($params['time_from'], $params['time_to']) = fn_create_periods($_REQUEST);
	...
	if ($report == 'operating_systems') {
		...
		$object_code = preg_replace("/\/$/", '', $object_code);
		...
		$text_conditions['page_load_speed'] = html_entity_decode($object_code);
		...
		$join['?:stat_requests'] = "?:stat_sessions.sess_id = ?:stat_requests.sess_id";
		$text_conditions['users_online'] = round(SESSION_ONLINE / 60) . ' ' . fn_get_lang_var('short_minute');
		...
		$condition .= db_quote(" AND ?:stat_requests.timestamp >= ?i AND ?:stat_requests.timestamp <= ?i", $params['time_from'], ($params['period'] == STAT_PERIOD_HOUR ? strtotime("+1 hour", $params['time_from']) : strtotime("+1 day", $params['time_from'])));
		$join['?:stat_requests'] = "?:stat_sessions.sess_id = ?:stat_requests.sess_id";
	}
	...
	if (!empty($_REQUEST['section'])) {
		fn_add_breadcrumb(fn_get_lang_var($_REQUEST['section']), "$index_script?dispatch=statistics.reports&reports_group=$_REQUEST[section]&report=$report");
...
//
	...
	if ($reports_group == 'system') {
		...
		if ($report == 'browsers') {
			$data['data'] = db_get_array("SELECT ?:stat_browsers.browser_id, CONCAT(?:stat_browsers.browser, ' ', ?:stat_browsers.version) AS label, COUNT(*) as count FROM ?:stat_sessions LEFT JOIN ?:stat_browsers ON ?:stat_sessions.browser_id = ?:stat_browsers.browser_id $filter_condition WHERE $where GROUP BY ?:stat_sessions.browser_id ORDER BY count DESC LIMIT ?i", $params['limit']);
		...
		Registry::set('navigation.tabs', array (
			'operating_systems' => array (
				'title' => fn_get_lang_var('operating_systems'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=system&report=operating_systems",
				'ajax' => true
			),
			'browsers' => array (
				'title' => fn_get_lang_var('browsers'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=system&report=browsers",
				'ajax' => true
			),
			'resolutions' => array (
				'title' => fn_get_lang_var('resolutions'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=system&report=resolutions",
				'ajax' => true
			),
		));
	...
	// Geo reports
		...
		if ($report == 'cities') {
			...
			$data['data'] = db_get_array("SELECT client_language, IF(client_language, CONCAT(language, ' (', client_language, ')'), language) AS label, COUNT(*) as count FROM ?:stat_sessions LEFT JOIN ?:stat_languages ON ?:stat_sessions.client_language = ?:stat_languages.lang_code $filter_condition WHERE $where GROUP BY client_language ORDER BY count DESC LIMIT ?i", $params['limit']);
			...
			$report = 'countries';
			$data['data'] = db_get_array("SELECT ?:stat_ips.country_code, country AS label, COUNT(*) as count FROM ?:stat_sessions LEFT JOIN ?:stat_ips ON ?:stat_sessions.ip_id = ?:stat_ips.ip_id LEFT JOIN ?:country_descriptions ON ?:stat_ips.country_code = ?:country_descriptions.code AND ?:country_descriptions.lang_code = ?s ?p WHERE ?p AND ?:stat_sessions.ip_id != 0 GROUP BY country_code ORDER BY count DESC LIMIT ?i", CART_LANGUAGE, $filter_condition, $where, $params['limit']);
		}

		Registry::set('navigation.tabs', array (
			'languages' => array (
				'title' => fn_get_lang_var('languages'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=geography&report=languages",
				'ajax' => true
			),
			'ip_addresses' => array (
				'title' => fn_get_lang_var('ip_addresses'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=geography&report=ip_addresses",
				'ajax' => true
			),
			'countries' => array (
				'title' => fn_get_lang_var('countries'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=geography&report=countries",
				'ajax' => true
			),
		));
		...
		if ($report == 'by_domain') {
			...
			$data['data'] = db_get_array("SELECT ?:stat_sessions.phrase_id, phrase AS label, COUNT(*) AS count FROM ?:stat_sessions LEFT JOIN ?:stat_search_phrases ON ?:stat_search_phrases.phrase_id = ?:stat_sessions.phrase_id $filter_condition WHERE $where AND ?:stat_sessions.phrase_id != 0 GROUP BY ?:stat_sessions.phrase_id, phrase ORDER BY count DESC LIMIT ?i", $params['limit']);

		} elseif ($report == 'came_to') {

			$_where_time = fn_get_sql_where_time('stat_requests', $params['time_from'], $params['time_to']);
			$where .= ' AND (request_type & '. STAT_ORDINARY_REQUEST .') = '. STAT_FIRST_REQUEST . (empty($_where_time) ? '' : " AND $_where_time ") . " AND ?:stat_sessions.engine_id != '0'";

			$join['?:stat_search_engines'] = "?:stat_search_engines.engine_id = ?:stat_sessions.engine_id";
			$filter_condition = fn_bild_sql_join($join);

			$data['data'] = db_get_array("SELECT ?:stat_sessions.engine_id, url AS label, COUNT(*) AS count FROM ?:stat_sessions LEFT JOIN ?:stat_requests ON ?:stat_requests.sess_id = ?:stat_sessions.sess_id $filter_condition WHERE $where GROUP BY ?:stat_sessions.engine_id, url ORDER BY count DESC LIMIT ?i", $params['limit']);
			...
			$data['data'] = db_get_array("SELECT ?:stat_sessions.engine_id, ?:stat_sessions.phrase_id, CONCAT('[', engine, '] ', phrase) AS label, COUNT(*) AS count FROM ?:stat_sessions LEFT JOIN ?:stat_search_phrases ON ?:stat_search_phrases.phrase_id = ?:stat_sessions.phrase_id $filter_condition WHERE $where AND ?:stat_sessions.phrase_id != 0 AND ?:stat_sessions.engine_id != 0 GROUP BY ?:stat_sessions.engine_id, engine, ?:stat_sessions.phrase_id, phrase ORDER BY count DESC LIMIT ?i", $params['limit']);

		} else { // all referrers

			$report = 'all_referrers';
			$data['data'] = db_get_array("SELECT referrer AS label, phrase, COUNT(*) AS count FROM ?:stat_sessions LEFT JOIN ?:stat_search_phrases ON ?:stat_search_phrases.phrase_id = ?:stat_sessions.phrase_id $filter_condition WHERE $where GROUP BY referrer, phrase ORDER BY count DESC LIMIT ?i", $params['limit']);
		}

		Registry::set('navigation.tabs', array (
			'by_domain' => array (
				'title' => fn_get_lang_var('by_domain'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=referrers&report=by_domain",
				'ajax' => true
			),
			'by_search_engine' => array (
				'title' => fn_get_lang_var('by_search_engine'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=referrers&report=by_search_engine",
				'ajax' => true
			),
			'search_words' => array (
				'title' => fn_get_lang_var('search_words'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=referrers&report=search_words",
				'ajax' => true
			),
			'came_from' => array (
				'title' => fn_get_lang_var('came_from'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=referrers&report=came_from",
				'ajax' => true
			),
			'came_to' => array (
				'title' => fn_get_lang_var('came_to'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=referrers&report=came_to",
				'ajax' => true
			),
			'all_referrers' => array (
				'title' => fn_get_lang_var('all'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=referrers&report=all_referrers",
				'ajax' => true
			),
		));
		...
		if ($report == 'titles_by_visits') {
			...
			$data['data'] = db_get_array("SELECT url AS label, title, COUNT(*) AS count FROM ?:stat_sessions LEFT JOIN ?:stat_requests ON ?:stat_requests.sess_id = ?:stat_sessions.sess_id AND ?:stat_requests.timestamp = ?:stat_sessions.timestamp $filter_condition WHERE $where AND url != '' GROUP BY url ORDER BY count DESC LIMIT ?i", $params['limit']);
			...
			$filter_condition = fn_bild_sql_join($join);
			$data['data'] = db_get_array("SELECT url AS label, title, COUNT(*) AS count FROM ?:stat_requests $filter_condition WHERE $where GROUP BY url ORDER BY count DESC LIMIT ?i", $params['limit']);
		}

		Registry::set('navigation.tabs', array (
			'titles_by_visits' => array (
				'title' => fn_get_lang_var('titles_by_visits'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=pages&report=titles_by_visits",
				'ajax' => true
			),
			'entry_points' => array (
				'title' => fn_get_lang_var('entry_points'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=pages&report=entry_points",
				'ajax' => true
			),
			'exit_points' => array (
				'title' => fn_get_lang_var('exit_points'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=pages&report=exit_points",
				'ajax' => true
			),
			'pages_by_visits' => array (
				'title' => fn_get_lang_var('pages_by_visits'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=pages&report=pages_by_visits",
				'ajax' => true
			),
		));
		...
		if ($report == 'site_attendance') {
			...
			$data['data'] = db_get_array("SELECT FROM_UNIXTIME(?:stat_sessions.timestamp, '%H') as hour, CONCAT(FROM_UNIXTIME(?:stat_sessions.timestamp, '%H'), ':00 ', ?s) AS label, COUNT(*) AS count FROM ?:stat_sessions ?p WHERE ?p GROUP BY hour ORDER BY hour", fn_get_lang_var('hours'), $filter_condition, $where);

		} elseif ($report == 'visit_time') {
			$join['?:stat_sessions'] = "?:stat_requests.sess_id=?:stat_sessions.sess_id";
			$filter_condition = fn_bild_sql_join($join);
			$data[STAT_VISIT_TIME] = array();
			...
			foreach ($range as $rng) {
				...
				$cnt = fn_calc_aggregate_functions("SELECT ?:stat_requests.sess_id FROM ?:stat_requests $filter_condition WHERE $where GROUP BY ?:stat_requests.sess_id HAVING (max(?:stat_requests.timestamp) - min(?:stat_requests.timestamp)) > " . ($rng_srt * 60) . ' ' . ($rng == $r_max ? '' : "AND (max(?:stat_requests.timestamp) - min(?:stat_requests.timestamp)) <= ". ($rng*60)), STAT_ROW_LIMIT);

				if ($cnt > 0) {
					$name = $rng == $r_min ? "<= $rng" : (($rng == $r_max ? '> '. $rng_srt/60 : ($rng_srt < 60 ? $rng_srt : $rng_srt/60) .' - '. ($rng < 60 ? $rng : $rng/60)));
					$data['data'][$rng] = array('label' => "$name " . fn_get_lang_var($rng < 60 ? 'short_minute' : 'short_hour'), 'count' => $cnt);
				}

				$rng_srt = $rng;
			}
			$data['average_duration'] = fn_this_day_begin() + round(fn_calc_aggregate_functions("SELECT max(?:stat_requests.timestamp) - min(?:stat_requests.timestamp) FROM ?:stat_requests $filter_condition WHERE $where GROUP BY ?:stat_requests.sess_id", STAT_ROW_LIMIT, 'AVG'));
			...
			$data['data'] = array(
				'O' => array('label' => fn_get_lang_var('old_visitors'), 'count' => count($old)),
				'N' => array('label' => fn_get_lang_var('new_visitors'), 'count' => count($new)),
			);
			...
			$data['data']['lt'] = array('label' => '< 1 '. fn_get_lang_var('short_second'), 'count' => $count);
			...
			foreach ($ranges as $range) {
				...
				for ($i = $from; $i < $to ; $i = $i + $step) {
					...
					$count = db_get_field("SELECT COUNT(*) AS count FROM ?:stat_requests LEFT JOIN ?:stat_sessions ON ?:stat_requests.sess_id = ?:stat_sessions.sess_id $filter_condition WHERE $where AND $loadtime_condition");
					$data['data'][$i] = array('label' => $i .' - ' . ($i + $step) . ' ' . fn_get_lang_var('short_second'), 'count' => $count);
			...
			$count = db_get_field("SELECT COUNT(*) AS count FROM ?:stat_requests LEFT JOIN ?:stat_sessions ON ?:stat_requests.sess_id = ?:stat_sessions.sess_id $filter_condition WHERE $where AND $loadtime_condition");
			$data['data']['gt'] = array('label' => '>= 1 '. fn_get_lang_var('short_minute'), 'count' => $count);
			...
			foreach ($range as $rng) {
				...
				if ($cnt > 0) {
					...
					$data['data'][$name] = array('label' => "$name " . fn_get_lang_var($rng_srt == 1 ? 'page' : 'pages'), 'count' => $cnt);
				}
		...
		Registry::set('navigation.tabs', array (
			'site_attendance' => array (
				'title' => fn_get_lang_var('site_attendance'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=audience&report=site_attendance",
				'ajax' => true
			),
			'visit_time' => array (
				'title' => fn_get_lang_var('visit_time'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=audience&report=visit_time",
				'ajax' => true
			),
			'repeat_new_visits' => array (
				'title' => fn_get_lang_var('repeat_new_visits'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=audience&report=repeat_new_visits",
				'ajax' => true
			),
			'page_load_speed' => array (
				'title' => fn_get_lang_var('page_load_speed'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=audience&report=page_load_speed",
				'ajax' => true
			),
			'total_pages_viewed_per_visitor' => array (
				'title' => fn_get_lang_var('total_pages_viewed_per_visitor'),
				'href' => "$index_script?dispatch=statistics.reports&reports_group=audience&report=total_pages_viewed_per_visitor",
				'ajax' => true
			),
		));
		...
		list($where, $join, $params) = fn_stat_filter_data($_REQUEST);
		$where .= " AND client_type = 'U'";
		$join = array('?:stat_sessions' => "?:stat_product_search.sess_id=?:stat_sessions.sess_id");
		...
		if ($report == 'search') {
			$data['data'] = db_get_array("SELECT ?:stat_product_search.search_string AS label, ?:stat_product_search.md5, DATE_FORMAT(FROM_UNIXTIME(?:stat_sessions.timestamp), '%Y-%m-%d') AS date, AVG(?:stat_product_search.quantity) AS quantity, COUNT(*) as count FROM ?:stat_product_search $filter_condition WHERE $where GROUP BY ?:stat_product_search.md5, DATE_FORMAT(FROM_UNIXTIME(?:stat_sessions.timestamp), '%Y-%m-%d') ORDER BY date DESC, count DESC LIMIT ?i", $params['limit']);

			if (!empty($data['data'])) {
				foreach ($data['data'] as $k => $v) {
					$data['data'][$k]['label'] = unserialize($v['label']);
					$data['data'][$k]['url'] = empty($data['data'][$k]['label']) ? '' : fn_build_query($data['data'][$k]['label']);
				}
			}

			$view->assign('product_features', fn_get_product_features(array(), CART_LANGUAGE)); // FIXME: common function
		}
		...
		if ($_period == STAT_PERIOD_HOUR) {
			$group_date = "DATE_FORMAT(FROM_UNIXTIME(?:stat_sessions.timestamp - " . TIME_DIFFERENCE . "), '%Y-%m-%d %H:00')";
		} elseif ($_period == STAT_PERIOD_MONTH) {
			$group_date = "DATE_FORMAT(FROM_UNIXTIME(?:stat_sessions.timestamp + " . TIME_DIFFERENCE . "), '%Y-%m-01')";
		} else {
			$_period = STAT_PERIOD_DAY;
			$group_date = "DATE_FORMAT(FROM_UNIXTIME(?:stat_sessions.timestamp + " . TIME_DIFFERENCE . "), '%Y-%m-%d')"; // Day
		}
		...
		$total = db_get_field("SELECT COUNT(DISTINCT $group_date) FROM ?:stat_sessions $filter_condition $where");
		$limit = fn_paginate($page, $total, Registry::get('settings.Appearance.admin_elements_per_page'), 'general_pagination_content');
		$data = array();
		...
		if ($chart_type != 'table') {
			$view->assign('chart_data', fn_stat_map_data($chart_type, $data['data'], array('total', 'visitors', 'hosts', 'robots'), array(fn_get_lang_var('total'), fn_get_lang_var('visits'), fn_get_lang_var('hosts'), fn_get_lang_var('robots')), true));
		}
		$view->assign('statistic_period', $_period);
		$view->assign('statistics_data', $data);
	}
}
...
foreach (array('general', 'system', 'geography', 'referrers', 'pages', 'audience', 'products') as $s) {
	$sections[$s] = array (
		'title' => fn_get_lang_var($s),
		'href' => "$index_script?dispatch=statistics.reports&reports_group=$s",
		'ajax' => true
	);
}
...
//
function fn_bild_sql_join($join)
{
	if (empty($join)) {
		return '';
	}
...
function fn_get_stat_requests_over_period($begin, $end, $order_field, $order_direction)
{
	...
	$fields = array('req_id', 'url', 'https', 'timestamp', 'loadtime');
	...
	if ($k = array_search('timestamp', $fields)) {
		$fields[$k] = "?:stat_requests.timestamp";
	}
	$_where_time = fn_get_sql_where_time('stat_requests', $params['time_from'], $params['time_to']);
	if (!empty($_where_time)) {
		$where .= " AND $_where_time";
	...
	if (!empty($requests)) {
		foreach ($requests as $req_id => $req) {
			if (!empty($req['url'])) {
				$requests[$req_id]['url'] = ((@$req['https'] == 'Y') ? Registry::get('config.https_location') : Registry::get('config.http_location')) . $req['url'];
			}
		}
	}
	return $requests;
...
function fn_stat_format_data($data)
{
	if (!empty($data['data'])) {
		...
		foreach ($data['data'] as $k => $v) {
			$data['data'][$k]['percent'] = round($v['count'] * 100 / $total, 2);
		}
	}
	...
	if ($aggregate == false) {
		foreach ($data as $k => $v) {
			foreach ($map as $field => $alias) {
				$result[$k][$alias] = ($v[$field] == '') ? fn_get_lang_var('undefined') : $v[$field];
			}
		}
	} else {
		foreach ($data as $k => $v) {
...
function fn_stat_get_visitors($params, $extra_join, $condition)
{
	...
	$join['?:stat_browsers'] = "?:stat_sessions.browser_id = ?:stat_browsers.browser_id";
	$join['?:stat_search_robots'] = "?:stat_sessions.robot_id = ?:stat_search_robots.robot_id";
	$join['?:stat_ips'] = "?:stat_sessions.ip_id = ?:stat_ips.ip_id";
	$join['?:stat_search_phrases'] = "?:stat_sessions.phrase_id = ?:stat_search_phrases.phrase_id";
	$join['?:stat_languages'] = "?:stat_sessions.client_language = ?:stat_languages.lang_code";
	...
	$data = db_get_hash_array("SELECT ?:stat_sessions.*, ?:stat_browsers.browser, ?:stat_browsers.version as browser_version, ?:stat_ips.*, phrase, ?:stat_languages.language, country, ?:stat_search_robots.name AS robot FROM ?:stat_sessions $filter_condition WHERE $where GROUP BY ?:stat_sessions.sess_id ". (empty($sorting) ? '' : "ORDER BY $sorting") ." $limit", 'sess_id');
	...
	if (!empty($data)) {
		...
		foreach ($data as $sess_id => $v) {
			if (isset($entity_data[$sess_id])) {
				$data[$sess_id]['url'] = (($entity_data[$sess_id]['https'] == 'Y') ? Registry::get('config.https_location') : Registry::get('config.http_location') ) . $entity_data[$sess_id]['url'];
				$data[$sess_id]['title'] = $entity_data[$sess_id]['title'];
			}

			if (isset($current_data[$sess_id])) {
				$data[$sess_id]['current_url'] = (($current_data[$sess_id]['https'] == 'Y') ? Registry::get('config.https_location') : Registry::get('config.http_location') ) . $current_data[$sess_id]['url'];
...
function fn_stat_product_search_data($search_params, $text_conditions = array())
{
	...
	if (!empty($search_params['feature'])) {
		$product_features = fn_get_product_features(array(), CART_LANGUAGE);
		$_data = array();
