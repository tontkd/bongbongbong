...
//
// $Id: func.php 7667 2009-07-07 10:50:59Z lexa $
//
...
// Get banner image
//
function fn_get_aff_banner_image_data($banner_id, $image_key = 'image')
{
	$image_data = fn_get_image_pairs($banner_id, 'common', 'M', 'aff_banners', true, false);
...
function fn_get_aff_banner_data($banner_id, $lang_code = CART_LANGUAGE, $avail_only = false)
{
	if (!empty($banner_id)) {
		...
		if (!empty($banner_data['type']) && $banner_data['type'] == 'G') {
			$image_data = fn_get_aff_banner_image_data($banner_data['banner_id']);
			if (!empty($image_data)) {
...
function fn_get_aff_banner_url($banner_data, $partner_id='', $lang_code = CART_LANGUAGE)
{
	...
	$banner_url = Registry::get('config.http_location') . '/' . Registry::get('config.customer_index') . "?dispatch=aff_banners.view&bid=$banner_data[banner_id]&sl=$lang_code";
	if (!empty($partner_id)) {
...
function fn_get_aff_banner_html($type, $banner_data, $mode = '', $partner_id = '', $lang_code = CART_LANGUAGE)
{
	...
	if ($banner_data['type'] == 'P') {
		...
		if (!empty($product_id)) {
			...
			$banner_data['content'] = '<div align="center" style="font-family: tahoma, arial; font-size: 12px;">'.fn_get_lang_var('warning').
				'</div><hr size="0" style="border-top: #3b67d2 1px solid;" /><span style="font-family: tahoma, arial; font-size: 10px; color: #3b67d2;">'.
				fn_get_lang_var('text_no_products_found').'</span>';
		}
	}
	...
	if (@$banner_data['type']=='G') {
		$image_data = fn_get_aff_banner_image_data($banner_data['banner_id'], 'icon');
		if (!empty($image_data)) {
			$banner_data = fn_array_merge($banner_data, $image_data);
		}
	}
	...
	if (in_array($type, array('iframe_content', 'js_content')) && !empty($partner_id) && $banner_correct) {
		fn_add_partner_action('show', $banner_data['banner_id'], $partner_id, '', array('R' => $_SERVER['HTTP_REFERER']));
	}
...
{
	$plans_list = db_get_hash_single_array("SELECT object_id, object FROM ?:affiliate_plans LEFT JOIN ?:common_descriptions ON plan_id=object_id AND lang_code = ?s AND object_table = 'affiliate_plans' WHERE status = 'A' ORDER BY object", array('object_id', 'object'), $lang_code);
...
{
	if (!empty($plan_id)) {
		$descr = db_get_field("SELECT object FROM ?:common_descriptions WHERE object_id = ?i AND lang_code = ?s AND object_table = 'affiliate_plans'", $plan_id, $lang_code);
...
function fn_get_partner_data($partner_id, $approved_only = false)
{
	...
	if (!empty($partner['plan_id'])) {
		$partner['plan'] = db_get_field("SELECT object FROM ?:common_descriptions WHERE object_id = ?i AND object_table = ?s AND lang_code = ?s", $partner['plan_id'], 'affiliate_plans', CART_LANGUAGE);
	}
...
function fn_update_partner_profile($user_id, $update_data)
{
	...
	$data = fn_check_table_fields($update_data, 'aff_partner_profiles');
	if (empty($data)) {
		return false;
	}
	...
	if (empty($is_profile)) {
		$result = db_query("INSERT INTO ?:aff_partner_profiles ?e", $data);
	} else {
		...
		$result = db_query("UPDATE ?:aff_partner_profiles SET ?u WHERE user_id = ?i", $data, $user_id);
		if (!empty($result) && !empty($data['plan_id']) && empty($old_plan_id) && empty($empty_init_balance)) {
...
function fn_update_partner_balance($partner_id, $amount, $action = '+')
{
	...
	if ($action == '-' && $amount > $partner['balance']) {
		$text = '<hr />';
		$text .= fn_get_lang_var('payment').": $amount<br />";
		$text .= fn_get_lang_var('affiliate').": $partner[firstname] $partner[lastname]<br />";
		$text .= fn_get_lang_var('balance_account').": $partner[balance]";
		fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('error_payout_gt_balance').$text);
	...
	if ($multi_tier_account) {
		...
		if ($action == 'sale' && !empty($additional_data['P'])) {
			...
			if ($p_value === false) {
				...
				if (!empty($product_data['main_category']['category_id'])) {
					$p_value = fn_get_payout_value($data['plan_id'], $product_data['main_category']['category_id'], 'category');
				}
			}
...
function fn_add_commissions_to_multi_tier_affiliates($action, $partner_id, $amount, $action_id, $customer_id = 0, $banner_id = 0, $additional_data=array())
{
	...
	foreach ($referrers as $k => $v) {
		...
		if ($add_action == true) {
			...
			fn_add_partner_action($action, $banner_id, $v['referrer_id'], $customer_id, $_add_data, $_amount / 100 * $plan_data['commissions'][$k], true);
		}
	}
...
function fn_add_sale_to_actions($order_id, $auth, $action = '', $partner_data)
{
	if (!empty($order_id) && $action != 'save') {
		...
		if (!empty($partner_data['partner_id'])) {
			$_data = array (
				'order_id' => $order_id,
				'type' => 'A', // affiliate information
				'data' => serialize(array('partner_id' => $partner_data['partner_id'])),
			);
			db_query("REPLACE INTO ?:order_data ?e", $_data);
		} else {
			db_query("DELETE FROM ?:order_data WHERE order_id = ?i AND type = 'A'", $order_id);
		}
		$order = fn_get_order_info($order_id);
		if (!empty($partner_data['partner_id']) && !empty($order['items'])) {
			if (empty($_plan_data)) {
				...
				foreach ($order['coupons'] as $coupon_code => $value) {
					foreach ($value as $promotion_id) {
						...
						if (!empty($promotion_id)) {
							fn_add_partner_action('use_coupon', @$partner_data['banner_id'], $partner_data['partner_id'], @$auth['user_id'], array('O'=>$order_id, 'D'=>$promotion_id), $order['subtotal']);
				...
				if (!empty($order['items'])) {
					foreach ($order['items'] as $product) {
						...
						if (!empty($subtotal) && !empty($product['product_id'])) {
							fn_add_partner_action('sale', @$partner_data['banner_id'], $partner_data['partner_id'], @$auth['user_id'], array('O' => $order_id, 'P' => $product['product_id']), $subtotal);
						}
					}
				}
					...
					foreach ($order['coupons'] as $coupon_code => $value) {
						foreach ($value as $promotion_id) {
							...
							if (!empty($coupon_subtotal)) {
								fn_add_partner_action('use_coupon', empty($partner_data['banner_id']) ? '' : $partner_data['banner_id'], $partner_data['partner_id'], empty($auth['user_id']) ? '' : $auth['user_id'], array('O' => $order_id, 'D' => $promotion_id), $coupon_subtotal);
							}
							unset($partner_data['promotion_ids'][$promotion_id]);
						}
					}
...
function fn_get_commissions_4_order($order_id, $paid = false, $lang_code = CART_LANGUAGE)
{
	...
	if (!empty($_aff_actions)) {
		foreach ($_aff_actions as $key => $act) {
			$_aff_actions[$key]['title'] = fn_get_lang_var($payout_types[$act['action']]['title'], $lang_code);
		}
	}
...
function fn_get_groups_list($status = 'A', $lang_code = CART_LANGUAGE)
{
	if (empty($status) || $status == 'D') {
		...
		$status_condition = " AND ?:aff_groups.status = 'A' ";
	}

	$groups_list = db_get_hash_array("SELECT ?:aff_groups.group_id, ?:aff_group_descriptions.name FROM ?:aff_groups LEFT JOIN ?:aff_group_descriptions ON ?:aff_groups.group_id = ?:aff_group_descriptions.group_id AND ?:aff_group_descriptions.lang_code = ?s WHERE 1 $status_condition ORDER BY name", 'group_id', $lang_code);
...
function fn_affiliate_place_order($order_id, $action)
{
	if (!empty($_SESSION['partner_data']) && !empty($_SESSION['partner_data']['is_payouts']) && $_SESSION['partner_data']['is_payouts'] != 'Y') {
		...
		fn_add_sale_to_actions($order_id, $_SESSION['auth'], $action, $_SESSION['partner_data']);
	}
...
function fn_affiliate_get_users($params, &$fields, &$sortings, &$condition, &$join)
{
	if (!empty($params['user_type']) && $params['user_type'] == 'P') {
		...
		if (!empty($params['approved'])) {
			$condition .= db_quote(" AND ?:aff_partner_profiles.approved = ?s", $params['approved']);
		}

		if (!empty($params['plan_id'])) {
			if ($params['plan_id'] == -1) {
				$condition .= db_quote(" AND (?:aff_partner_profiles.plan_id = 0 OR ISNULL(?:aff_partner_profiles.approved))");
			} else {
				$condition .= db_quote(" AND ?:aff_partner_profiles.plan_id = ?i", $params['plan_id']);
			}
		}

		if (!empty($params['referrer_partner_id'])) {
			$condition .= db_quote(" AND ?:aff_partner_profiles.referrer_partner_id = ?i", $params['referrer_partner_id']);
		}

		$join .= db_quote(" LEFT JOIN ?:aff_partner_profiles ON ?:aff_partner_profiles.user_id = ?:users.user_id LEFT JOIN ?:common_descriptions ON ?:aff_partner_profiles.plan_id = ?:common_descriptions.object_id AND ?:common_descriptions.object_table = 'affiliate_plans' AND ?:common_descriptions.lang_code = ?s", CART_LANGUAGE);
	}

	return true;
}

function fn_affiliate_get_profile_fields($location, &$select, &$condition)
{
	if ($location == 'P') { // this is fields for an affiliate
...
function fn_affiliate_auth_routines(&$status, &$user_data)
{
	if (!empty($user_data['user_id']) && $user_data['user_type'] == 'P') {
		...
		if ((empty($user_data['approved']) || $user_data['approved'] == 'N')) {
			fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('error_account_not_approved'));
		}

		if (!empty($user_data['approved']) && $user_data['approved'] == 'D') {
			fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('error_account_declined'));
		}
	}
