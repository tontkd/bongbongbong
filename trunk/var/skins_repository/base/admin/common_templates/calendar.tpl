...
<input type="text" id="{$date_id}" name="{$date_name}" class="input-text{if $date_meta} {$date_meta}{/if}" value="{if $date_val}{$date_val|date_format:"`$date_format`"}{/if}" {$extra} size="10" />&nbsp;<img src="{$images_dir}/icons/calendar.gif" class="cm-combo-on cm-combination calendar-but" id="sw_{$date_id}_picker" title="{$lang.calendar}" alt="{$lang.calendar}" />
