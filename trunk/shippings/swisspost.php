...
//
// $Id: swisspost.php 7774 2009-07-31 09:47:01Z zeke $
//
...
function fn_get_swisspost_rates($code, $weight_data, $location, &$auth, $shipping_settings, $package_info, $origination)
{
	if ($shipping_settings['swisspost_enabled'] != 'Y') {
		return false;
	}

	$path = DIR_SHIPPING_FILES . 'swisspost/' . $code;
	...
	if (!empty($location['country']) && file_exists($path) == true) {
		...
		if (in_array($location['country'], $local_countries) && strpos($code, 'int') !== false) {
			return defined('SHIPPING_DEBUG') ? array('error' => "Cannot use this shiipping method for the private delivery") : false;
		} elseif (!in_array($location['country'], $local_countries) && strpos($code, 'prv') !== false) {
			return defined('SHIPPING_DEBUG') ? array('error' => "Cannot use this shiipping method for the international delivery") : false;
		}
		...
		$tariff_array = (($num - 1) == 2) ? fn_get_swisspost_zones(ZONE_2) : ((($num - 1) == 5) ? fn_get_swisspost_zones(ZONE_5) : array());
		...
		if ($price == 0) {
			$errors[] = 'The package is too heavy';
		} else {
			// additional services
			$add_path = DIR_SHIPPING_FILES . '/swisspost/additional_services.csv';
			...
			if (file_exists($add_path) == true) {
				...
				while ($add_data = fgetcsv ($add_fp, $max_line_size, $delimiter)) {
					...
					if (Registry::get("settings.Shippings.sp.$add_rates_data[service]") == 'Y' && $add_rates_data['price'] > 0 && strpos($add_rates_data['service'], $type) === 0 && (($add_rates_data['service'] == 'pp_cash_on_delivery' && strpos($code, '_ec_') > 0) || ($add_rates_data['service'] != 'pp_cash_on_delivery'))) {
						$price += $add_rates_data['price'];
					}
				}
				fclose($add_fp);
			} else {
				$errors[] = 'Cannot open the additional_services.csv file';
			}
		}
	} else {
		$errors[] = 'Cannot open the ' . $code . ' file';
	}

	if (!empty($price)) {
		return array('cost' => $price);
	} else {
