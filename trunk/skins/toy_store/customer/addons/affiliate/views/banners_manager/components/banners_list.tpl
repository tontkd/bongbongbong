{* $Id: banners_list.tpl 7279 2009-04-15 11:13:51Z lexa $ *}

<form action="{$index_script}" method="post" name="manage_{$prefix}_banners_form">
...
<tr {cycle values=" ,class=\"table-row\""} valign="top">
	...
	<td>
		...
		{if $smarty.request.banner_type == "P"}<a href="{$index_script}?dispatch=banners_manager.select_product&amp;banner_id={$banner_id}">{$lang.banner_code_for_some_products}</a>{/if}
	</td>
	{if $smarty.request.banner_type != "P"}
	<td>
