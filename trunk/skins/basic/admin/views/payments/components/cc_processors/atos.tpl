{* $Id: atos.tpl 7719 2009-07-16 04:46:38Z zeke $ *}
...
{/if}
{assign var="auto_url" value="`$config.http_location`/`$config.customer_index`?dispatch=payment_notification.result&payment=atos"}
{assign var="ok_url" value="`$config.http_location`/`$config.customer_index`?dispatch=payment_notification.process&payment=atos"}
...
<p><strong>{$lang.text_atos_warning}</strong></p>
{/if}
<hr />
