...
//
// $Id: settings_dev.php 7763 2009-07-29 13:19:43Z alexions $
//
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update') {
		...
		if (!empty($_REQUEST['dev_descr'])) {
			foreach ($_REQUEST['dev_descr'] as $k => $v) {
				if (!empty($v)) {
					db_query("UPDATE ?:settings_descriptions SET description = ?s WHERE object_id = ?s AND object_type = 'O' AND lang_code = ?s", trim($v), $k, CART_LANGUAGE);
				}
			}
		}
		...
		if (!empty($_REQUEST['dev_option_name'])) {
			foreach ($_REQUEST['dev_option_name'] as $k => $v) {
				if (!empty($v)) {
					db_query("UPDATE ?:settings SET option_name = ?s WHERE option_id = ?i", strtolower(strtr(trim($v), array('-'=>'_',' '=>'_'))), $k);
				}
			}
		}
	...
	if ($mode == 'dev_add_subsection') {
		...
		if (!empty($dev['subsection_id'])) {
			$dev['subsection_id'] = strtolower(strtr(trim($dev['subsection_id']), array('-'=>'_',' '=>'_')));
	...
	if ($mode == 'dev_add_option') {
		...
		$dev['option_name'] = strtolower(strtr(trim($dev['option_name']), array('-'=>'_',' '=>'_')));
	...
	// \FOR DEVELOPMENT PURPOSES ONLY!!!
	return array(CONTROLLER_STATUS_OK, "$index_script?dispatch=settings_dev.manage&section_id=$section_id");
}
...
if ($mode == 'manage') {
	...
	$options = db_get_hash_multi_array("SELECT ?:settings.*, ?:settings_descriptions.description FROM ?:settings $descr WHERE ?:settings.section_id = ?s ORDER BY ?:settings.position", array('subsection_id'), $section_id);
