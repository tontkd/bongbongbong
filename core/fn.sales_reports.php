...
//
// $Id: fn.sales_reports.php 7502 2009-05-19 14:54:59Z zeke $
//
...
function fn_check_intervals($interval, $time_from, $time_to, $limit = 0)
{
	...
	if ($intervals['0']['value'] != 0) {
		...
		while ($time_to > $time_end) {
			...
			$time_end += ($intervals['0']['interval_id'] == '7') ? (mktime(0, 0, 0, date("m", $time_end)+1  , date("d", $time_end), date("Y", $time_end)) - $time_end) : $temp['value'];
		...
		if ($table['auto'] == "Y") {
			...
			while ($i < $limit) {
				...
				if ($new_element['code'] == 'order') {
					...
					$new_element['description'] = ($table['type'] != 'T') ? ($_desc_id . fn_get_lang_var('order') . '#' . $o_id) : ('<a href="' . Registry::get('config.admin_index') . "?dispatch=orders.details&order_id=$o_id" . '">'.$i.'. ' . fn_get_lang_var('order') . ' #' . $o_id . "</a>");
				...
				elseif ($new_element['code'] == 'status') {
					...
					$new_element['description'] = ($table['type'] != 'T') ? ("$i. $order_status_descr[$status]") : ('<a href="' . Registry::get('config.admin_index') . "?dispatch=orders.managesearch_orders=Y&status=$status&period=C&$time_link" . '">'."$i. $order_status_descr[$status]</a>");
				...
				elseif ($new_element['code'] == 'payment') {
					...
					$_descr = (strlen($pay_name) > $l_l) ? $_desc_id . substr($pay_name, 0, $l_l) . "..." : $_desc_id . $pay_name;
					$new_element['description'] = ($table['type'] != 'T') ? $_descr : ('<a href="' . Registry::get('config.admin_index') . "?dispatch=payments&highlight=$pay_id" . '">'."$_descr</a>");
				...
				elseif ($new_element['code'] == 'location') {
					...
					$_descr = (strlen($c_name) > $l_l) ? $_desc_id . substr($c_name, 0, $l_l) . "..." : $_desc_id . $c_name;
				...
				elseif ($new_element['code'] == 'user') {
					...
					$_descr = (strlen($u_name) > $l_l) ? $_desc_id . substr($u_name, 0, $l_l) . "..." : $_desc_id . $u_name;
					$new_element['description'] = ($table['type'] != 'T') ? $_descr : ('<a href="' . Registry::get('config.admin_index') . "?dispatch=profiles.update&user_id=$u_id" . '">'."$_descr</a>");
				...
				elseif ($new_element['code'] == 'category') {
					...
					$_descr = (strlen($c_name) > $l_l) ? $_desc_id . substr($c_name, 0, $l_l) . "..." : $_desc_id . $c_name;
					$new_element['description'] = ($table['type'] != 'T') ? $_descr : ('<a href="' . Registry::get('config.admin_index') . "?dispatch=categories.update&category_id=$c_id" . '">'."$_descr</a>");
					if (empty($c_id)) {
						$new_element['description'] = "$i. ".fn_get_lang_var('deleted');
					}
				...
				elseif ($new_element['code'] == 'product') {
					...
					$_descr = (strlen($p_name) > $l_l) ? $_desc_id . substr($p_name, 0, $l_l) . "..." : $_desc_id . $p_name;
					$new_element['description'] = ($table['type'] != 'T') ? $_descr : ('<a href="' . Registry::get('config.admin_index') . "?dispatch=products.update&product_id=$p_id" . '">'."$_descr</a>");
		...
		foreach ($table['intervals'] as $interval) {
			...
			if ($table['display'] == 'order_amount') {
				...
				$data[$a][$b] = db_get_field("SELECT SUM(shipping_cost) FROM ?:orders WHERE $element[request] AND $interval[request] $order_ids");
				 ...
				 $data[$a][$b] = db_get_field("SELECT SUM(discount) FROM ?:orders WHERE $element[request] AND $interval[request] $order_ids");
...
function fn_reports_get_conditions($conditions)
{
	...
	foreach ($conditions as $key => $value) {
		...
		if ($key == "order") {
			foreach($value as $v) {
				$result[$key]['objects'][] = array(
					'href' => Registry::get('config.admin_index') . '?dispatch=orders.details&order_id=' . $v,
					'name' => '#' . $v
				);
			}
		...
		elseif ($key == "user") {
			foreach($value as $v) {
				$result[$key]['objects'][] = array(
					'href' => Registry::get('config.admin_index') . '?dispatch=profiles.update&user_id=' . $v,
					'name' => $v,
				);
			}
			...
			foreach($value as $k => $v) {
				$result[$key]['objects'][] = array(
					'href' => Registry::get('config.admin_index') . '?dispatch=categories.update&category_id=' . $v,
			...
			foreach($value as $v) {
				$result[$key]['objects'][] = array(
					'href' => Registry::get('config.admin_index') . '?dispatch=products.update&product_id=' . $v,
					'name' => $v,
				);
			}
...
//
function fn_get_amchart_settings($type, $title = '', $setting_type = '')
{
	$settings = '';
	if (empty($type) || !in_array($type, array('pie', 'column'))) {
		$type == 'pie';
	}
	$setting_type = (!empty($setting_type)) ? $setting_type : $type;
	$settings = fn_get_contents(DIR_LIB . 'amcharts' . '/' . "am$type" . '/' . "am{$setting_type}_settings.xml");
	$settings = str_replace('[title_text]', $title, $settings);
	return $settings;
}
