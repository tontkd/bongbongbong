...
//
// $Id: fn.locations.php 7502 2009-05-19 14:54:59Z zeke $
//
...
function fn_get_state_name($state_code, $country_code, $lang_code = CART_LANGUAGE)
{
	...
	if (empty($states[$country_code][$state_code])) {
...
function fn_get_browser_language($languages = array())
{
	...
	if (!empty($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
		$preg_string = strtolower(implode('|' , array_keys($languages)));
	...
	if (empty($browser_language)) {
		reset($languages);
		$browser_language = key($languages);
	}
...
function fn_get_localizations($lang_code = CART_LANGUAGE, $status_check = false)
{
	...
	return db_get_hash_array("SELECT a.localization_id, a.status, b.localization, a.is_default FROM ?:localizations as a LEFT JOIN ?:localization_descriptions as b ON a.localization_id = b.localization_id AND b.lang_code = ?s ?p ORDER BY localization", 'localization_id', $lang_code, $status_condition);
}
...
function fn_get_simple_localizations($lang_code = CART_LANGUAGE, $status_check = false)
{
	...
	return db_get_hash_single_array("SELECT a.localization_id, b.localization FROM ?:localizations as a LEFT JOIN ?:localization_descriptions as b ON a.localization_id = b.localization_id AND b.lang_code = ?s ?p ORDER BY localization", array('localization_id' , 'localization'), $lang_code, $status_condition);
}
...
function fn_get_localizations_condition($db_field, $and = true, $localization = '')
{
	...
	return (empty($localization) || AREA != 'C') ? '' : (($and == true) ? ' AND' : '') . " (FIND_IN_SET($localization , $db_field))";
}
